...
export default function LanguageField() {
  ...
  const [selected, setSelected] = React.useState<Languages[]>(watch('language'));
  const [inputValue, setInputValue] = React.useState("");
  const { languages } = selectOptions();

  const handleUnselect = React.useCallback((framework: Languages) => {
    setSelected(prev => prev.filter(s => s.language_name !== framework.language_name));
  }, []);
              ...
              >
                <div className="flex gap-1 flex-wrap">
                  {selected.map((framework) => {
                    return (
                      <Badge key={framework.language_code} className="bg-white border-primary border" variant="secondary">
